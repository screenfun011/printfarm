...
import { usePrinters, useAddPrinter, useUpdatePrinter, useRemovePrinter } from '../hooks'
...
jest.mock('@/lib/api-client', () => ({
  printersApi: {
    ...
    add: jest.fn().mockResolvedValue({ id: '2', name: 'Printer B', model: 'p1s', serialNumber: 'DEF', status: 'offline', createdAt: '2024-01-01' }),
    update: jest.fn().mockResolvedValue({ id: '1', name: 'Updated', status: 'offline', isActive: true }),
...
describe('useAddPrinter', () => {
  it('poziva printersApi.add', async () => {
    const { printersApi } = require('@/lib/api-client')
    const { result } = renderHook(() => useAddPrinter(), { wrapper: createWrapper() })

    result.current.mutate({ name: 'Novi', model: 'a1', serialNumber: 'XYZ', ipAddress: '192.168.1.2', accessCode: '12345678' })
